...
class ShopService {
  ...
  async searchAndFilter(filter: ShopFilterDto): Promise<PagedResult<GetShop>> {
    ...
    const response = await axiosClient.get(`/api/Shops/search?${params.toString()}`);
    return response.data;
  }
  ...
  async recalculateRating(id: string): Promise<ServiceResponse> {
    const response = await axiosClient.post(`/api/Shops/${id}/recalculate-rating`);
    return response.data;
  }
  ...
  async getFeatured(limit: number = 6, city?: string): Promise<GetShop[]> {
    try {
      ...
      // Fallback: lấy active rồi cắt theo limit
  ...
  async getRevenueSummary(
    ...
    const normalizeDateTime = (s?: string, isEnd?: boolean): string | undefined => {
    ...
    const buildQuery = (p: RevenueSummaryParams) => {
      ...
      if (typeof p.onlyPaid === "boolean") q.append("onlyPaid", String(p.onlyPaid));
    ...
    try {
      ...
      return {
        succeeded: false,
        message: lastError?.message || aggErr?.message || 'Revenue summary unavailable',
        data: {
          totalRevenue: 0,
          byMethod: { cash: 0, cod: 0, bank: 0, wallet: 0 },
          orders: { totalOrders: 0, paidOrders: 0, refundedOrders: 0 },
...
async function computeRevenueSummaryFromOrders(
  ...
  const methods = Array.isArray(params.paymentMethod) ? params.paymentMethod : undefined;
  ...
  const filtered = orders.filter((o) => {
    ...
    const paidOk = typeof params.onlyPaid === 'boolean'
      ? (o.isPaid ?? o.paymentStatus === 'Paid') === params.onlyPaid
      : true;
    const methodOk = methods && methods.length ? methods.includes(o.paymentMethod) : true;
  ...
  let paidOrders = filtered.filter((o) => (o.isPaid ?? o.paymentStatus === 'Paid')).length;
  let refundedOrders = filtered.filter((o) => o.paymentStatus === 'Failed' || o.status === 'Canceled').length;

  // Timeseries buckets
  const buckets: Record<string, { revenue: number; orders: number; paidOrders: number }> = {};
  ...
  for (const o of filtered) {
    ...
    const created = new Date(o.createdAt);
    const bucketDate = groupBy === 'week' ? startOfISOWeek(created) : new Date(created);
    const dayKey = formatDay(bucketDate);

    if (!buckets[dayKey]) buckets[dayKey] = { revenue: 0, orders: 0, paidOrders: 0 };
